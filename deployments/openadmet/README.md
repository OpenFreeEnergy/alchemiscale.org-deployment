# alchemiscale.org - openadmet deployment

The [`openadmet`](/deployments/openadmet) deployment is reachable by users at [https://api.openadmet.alchemiscale.org](https://api.openadmet.alchemiscale.org/redoc).
...
Switch to the *Deployed Tag* listed for the `openadmet` deployment in the table [here](/README.md).

    $ git checkout <deployed-tag>

Create a conda environment using, e.g. [`micromamba`](https://github.com/mamba-org/micromamba-releases)::

    $ micromamba create -f deployments/openadmet/conda-envs/alchemiscale-client.yml
